...
export class EC2 extends Construct {
    constructor(scope: Construct, id: string, stage: string, ecr: ECR) {
        ...
        const keyPair = new ec2.CfnKeyPair(this, 'KeyPair', {
            keyName: `${stage}-archimedes-backend-keypair`,
        });
        ...
        const logging = new ecs.AwsLogDriver({ streamPrefix: `${stage}-archimedes-backend-service-task-definition` })

        // Create a task definition with its own elastic network interface
        const taskDefinition = new ecs.Ec2TaskDefinition(this, 'task-definition-2', {
        ...
        // Create the service
        const service = new ecs.Ec2Service(this, 'ecs-backend-service', {
            cluster,
            taskDefinition,
            serviceName: `${stage}-ecs-archimedes-backend-service`,
